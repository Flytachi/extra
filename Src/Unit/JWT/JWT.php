...
class JWT {
     ...
     *
     * @throws JWTError If the provided algorithm is not supported or if there is an error during token generation.
     */
    public static function encode(array $payload, int $expireTimeInMinutes = null, int $notBeforeInMinutes = null, string $algorithm = 'HS256'): string
    {
        if (!isset(self::$algorithms[$algorithm])) throw new JWTError("Algorithm '{$algorithm}' not supported");
        ...
        try {
            ...
            if (in_array($algorithm, ['RS256', 'RS384', 'RS512'])) {
                ...
                openssl_sign("$base64UrlHeader.$base64UrlPayload", $signature, $privateKey, self::$algorithms[$algorithm]);
                openssl_free_key($privateKey);
            } else {
                $secret = self::$secretKey;
                $signature = hash_hmac(self::$algorithms[$algorithm], "$base64UrlHeader.$base64UrlPayload", $secret, true);
            }

            $base64UrlSignature = self::base64UrlEncode($signature);
            return "$base64UrlHeader.$base64UrlPayload.$base64UrlSignature";
        } catch (\Throwable $exception) {
            throw new JWTError("Encoding Error '{$exception->getMessage()}");
        }
    }
     ...
     * @throws JWTError if the token is not yet valid or has expired, if the algorithm is not supported,
    ...
    public static function decode(string $token): array
    {
        try {
            ...
            if (isset($payload['nbf']) && time() < $payload['nbf'])
                throw new JWTError('The token is not yet valid');

            if (isset($payload['exp']) && $payload['exp'] <= time())
                throw new JWTError('Token has expired');

            if (!isset(self::$algorithms[$header['alg']])) throw new JWTError("Algorithm '{$header['alg']}' not supported");
            $algorithm = $header['alg'];

            if (in_array($algorithm, ['RS256', 'RS384', 'RS512'])) {
                $publicKey = openssl_get_publickey(file_get_contents(self::$RSAPublicKeyPath));
                $result = openssl_verify("$headerEnc.$payloadEnc", self::base64UrlDecode($signatureEnc), $publicKey, self::$algorithms[$algorithm]);
                openssl_free_key($publicKey);
                if ($result !== 1) throw new JWTError('Invalid signature. The token signature could not be verified');
            } else {
                $secret = self::$secretKey;
                $signatureCheck = self::base64UrlEncode(hash_hmac(self::$algorithms[$algorithm], "$headerEnc.$payloadEnc", $secret, true));
                if ($signatureCheck !== $signatureEnc) throw new JWTError('Invalid signature. The token signature could not be verified');
            }

            return $payload;
        } catch (\Throwable $exception) {
            throw new JWTError("Decoding Error '{$exception->getMessage()}");
        }
    }
