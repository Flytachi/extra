 ...
 * It includes mechanisms to generate random strings, performing weighted selection from an array, and implementing a binary search.
 *
 * The methods provided by `Algorithm` include:
 *
 * - `random(int $length, ?string $alphabet = null): string`: Generates a random string of the specified length using an optional alphabet.
 * - `weightedRandomLite(array $values, array $weights): mixed`: Randomly selects one element from a given array based on their weights in a lightweight manner. This function is optimized for a large number of elements.
 * - `weightedRandom(array $values, array $weights): mixed`: Randomly selects one element from a given array based on their weights. This function is optimized for a large number of elements.
